      ...
      <table className="w-full text-sm text-gray-400">
        ...
        <thead>
          <tr className="border-b border-gray-600">
            ...
            <th className="text-center py-1">Asset</th>          
          ...
          {assets.map((asset) => (
            <tr key={asset.id || asset.currency}>
              ...
              <td className="text-right">
                {loading ? <Skeleton width={50} height={16} baseColor="var(--skeleton-base)" highlightColor="var(--skeleton-highlight)"/> : rates?.[asset.price] ? `$${(1 / Number(rates[asset.price])).toFixed(4)}` : "-"}
              </td>
            </tr>
          ))}
        </tbody>
