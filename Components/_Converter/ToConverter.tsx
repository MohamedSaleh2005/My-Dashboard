...
export default function ToConverter() {
    ...
    useEffect(() => {
        ...
        if (rate) {
            ...
            setOneResult({ price: (1 / price), currency: currency });
        }
        ...
        <div className="flex flex-col w-[40%]">
            ...
            <div className="relative">
                <div className="Special border-gray-400/60 border p-2 text-sm flex justify-between items-center rounded-sm mt-1">
                    ...
                    <span onClick={() => setOpening(!Opening)} className={`cursor-pointer transition-all duration-300 ${Opening ? "rotate-180" : ""}`}>
                        <FaAngleDown />
                    </span>
                </div>
                ...
                <div className={`absolute mt-1 w-full border rounded-sm overflow-y-scroll h-16 Special no-scrollbar ${Opening ? "Open" : "Close"}`}>
                    {CurItems.map((cur: CurItem) => (
                        <div key={cur.id} onClick={() => ChooseCurrency(cur)} className="flex items-center py-1 px-3 gap-3 cursor-pointer text-sm">
