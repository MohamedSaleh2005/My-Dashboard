...
export default function CurrencyRow({ item, rates, oldPrices, loading }: Props) {
  ...
  if (oldPrice !== undefined) trend = price > oldPrice ? "up" : price < oldPrice ? "down" : "same"
  ...
  return (
    <div className='w-full border-b-2 border-[rgba(75,192,192,0.32)] h-10 mt-2 flex justify-between items-center px-3 text-sm pr-8'>
      ...
      <span className={`w-15 ${trend === "up" ? "text-green-500" : trend === "down" ? "text-red-500" : "text-gray-400"}`}>
        {loading 
          ? <Skeleton width={40} height={16} baseColor="var(--skeleton-base)" highlightColor="var(--skeleton-highlight)" />
          : trend === "up" ? `▲ ${percent.toFixed(2)}%` 
          : trend === "down" ? `▼ ${percent.toFixed(2)}%` 
          : "0.00%"
        }
      </span>

      <AddFavourite item={item} /> {/* لا نحتاج بروبس للفيفوريت أو التوغل بعد الآن */}
    </div>
  )
}
