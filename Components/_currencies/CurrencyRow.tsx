...
export default function CurrencyRow({ item }: { item: any }) {
  ...
  return (
    <div className='w-full border-b-2 border-[rgba(75,192,192,0.32)] h-10 mt-2 flex justify-between items-center px-3 text-sm pr-8'>
      ...
      <span className='w-10'><img alt='Currency' src={item.name?.icon} className='w-5 h-5 rounded-full' /></span>
      ...
      <span className={`w-15 ${trend === "up" ? "text-green-500" : trend === "down" ? "text-red-500" : "text-gray-400"}`}>
        {loading ? (<Skeleton width={40} height={16} />) : trend === "up" ? (`▲ ${percent.toFixed(2)}%`) : trend === "down" ? (`▼ ${percent.toFixed(1)}%`) : ("0.00%")}
      </span>
